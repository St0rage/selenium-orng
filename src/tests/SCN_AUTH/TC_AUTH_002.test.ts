...
beforeAll(async () => {
  ...
  const data = await TestDataUtil.loadTestData<SCN_AUTH>(scenarioName);
  testData = TestDataUtil.getTestDataByTestCaseName(data, testCaseName);
...
test(
  ...
  async () => {
    ...
    expect(await loginPage.checkAlertInvalidCredentialExist()).to.be(true);
    await driverUtil.screenshoot(
      "Expect Login Fail",
      "Expected : Memastikan Gagal Login Dengan User Salah\nActual : Berhasil Memastikan Gagal Login Dengan User Salah",
      Status.Passed
    );
  },
  60000
